...
const ConfigurationPage = () => {
  ...
  useEffect(() => {
    ...
    if (user?.userId && isAdmin) {
      void loadConfiguration(user.userId);
    }
  }, [user?.userId, isAdmin]);
    ...
    return (
      <div className="p-6">
        <Alert variant="destructive">
          ...
          <AlertDescription>{t("configuration.noPermission")}</AlertDescription>
        </Alert>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-3xl font-bold text-brand-primary">{t("configuration.title")}</h1>
      ...
      <Card className="bg-white border border-playBlueLight/30">
        <CardHeader>
          ...
          <CardDescription>{t("configuration.logo.description")}</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          {configuration?.logoUrl && !imageError ? (
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-muted-foreground">
                  ...
                  {new Date(configuration.updatedAt).toLocaleDateString("es-ES", {
                    day: "2-digit",
                    month: "long",
                    year: "numeric",
                  })}
            ...
            <Alert variant="destructive">
              <AlertCircle className="h-4 w-4" />
              <AlertDescription>{t("configuration.logo.loadError")}</AlertDescription>
            </Alert>
          ) : (
            <Alert>
              <ImageIcon className="h-4 w-4" />
              <AlertDescription>{t("configuration.logo.noLogo")}</AlertDescription>
            </Alert>
          )}
          ...
          <div className="bg-blue-50 p-4 rounded-lg text-sm text-blue-900 space-y-2">
            ...
            <ul className="list-disc list-inside space-y-1 text-xs">
              <li>{t("configuration.logo.infoItems.header")}</li>
              <li>{t("configuration.logo.infoItems.accessControl")}</li>
              <li>{t("configuration.logo.infoItems.delete")}</li>
              <li>{t("configuration.logo.infoItems.immediate")}</li>
            </ul>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="bg-white border border-playBlueLight/30">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-brand-primary">
            ...
            Gestion de sedes y permisos PRL
          </CardTitle>
          <CardDescription>
            Configura sedes, asignaciones de empresas y permisos de usuarios por sede.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Button asChild variant="outline">
            <Link href="/dashboard/configuration/sites">
              Abrir configuracion de sedes
      ...
      <Card className="bg-white border border-playBlueLight/30 opacity-50">
        <CardHeader>
          ...
          <CardDescription>{t("configuration.moreSettings.description")}</CardDescription>
        </CardHeader>
      </Card>
    </div>
  );
